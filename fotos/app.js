...
const state = {
  ...
  activePhotoId: null, // id de la foto mostrada actualmente en el editor full-size del Paso 1
...
// Fotos no tiene "carillas" — usamos la cantidad total de copias como medida
// de volumen para la anticipación de producción (mismo criterio que ya usaba
// selectFecha()/renderResumenFinal() antes de esta reescritura).
const entrega = createEntregaStep({
  mount: document.getElementById('entregaMount'),
  categoria: CATEGORIA,
  carillasProvider: () => {
    let total = 0;
    files.forEach(entry => { total += calcularFoto(entry).copias; });
    return total;
  },
...
function addFiles(fileListObj) {
  ...
  accepted.forEach(f => {
    ...
    // foto (incluida una que no se esté viendo) al confirmar el Paso 1.
    ...
    const entry = {
      ...
      // el usuario confirma el Paso 1 tocando "Continuar" (ver subirTodasLasFotos()).
...
// cuando el usuario confirma el Paso 1 (ver subirTodasLasFotos()).
...
// Se llama al tocar "Continuar" en el Paso 1. Sube todas las fotos que todavía
...
document.getElementById('btnOverlayReintentar').addEventListener('click', async () => {
  ...
  if (ok) {
    ...
    // ahora quedó todo subido y sigue en el Paso 1, completamos la acción
    // que había pedido en vez de dejarlo varado con todo listo pero quieto.
    if (state.step === 1 && stepValido(1)) goToStep(2);
  } else {
...
// (Factura A). Sólo informativo en el frontend — ver la misma nota en
// impresion-rapida/app.js (no hay integración de facturación/AFIP real
// todavía).
...
// resumen colapsado con "Editar" en vez de un formulario vacío — mismo
// patrón que impresion-rapida/app.js.
...
async function renderResumenFinal() {
  ...
  try {
    const qs = new URLSearchParams({ zona_id: resultadoEntrega.zona.id, categoria: CATEGORIA, carillas: unidadesTotal });
...
/* =========================================================
   NAVEGACIÓN DEL WIZARD (4 pasos: Fotos, Entrega, Datos, Pago)
   ========================================================= */
function stepValido(n) {
  switch (n) {
    case 1: return files.size > 0 && [...files.values()].every(e => !e.subiendo && !e.errorSubida);
...
function updateNavState() {
  ...
  // "OK" reemplaza a la flecha únicamente dentro del editor de fotos (Paso 1
  ...
  const enEditorDeFotos = state.step === 1 && files.size > 0;
  ...
  // fotos y también en los Pasos 2 (Entrega) y 3 (Datos) — el resto de los
  // pasos conserva el texto completo.
  const soloFlecha = enEditorDeFotos || state.step === 2 || state.step === 3;
...
function goToStep(n) {
  ...
  // aplica mientras se está en el Paso 1 con fotos cargadas — en cualquier
  // otro paso el wizard vuelve a verse con su nav completa normal.
  setModoEditorFotos(n === 1 && files.size > 0);
  if (n === 2) {
...
// ya en el Paso 1. Acá las fotos pueden llegar con el usuario todavía en
// otro paso (por ejemplo, si vuelve atrás), así que las dejamos en espera y
// las soltamos recién cuando efectivamente entra al Paso 1 — enganchando
// goToStep sin tocar su implementación original.
let pendingSharedFiles = null;
const _goToStepOriginal = goToStep;
goToStep = function (n) {
  _goToStepOriginal(n);
  if (n === 1 && pendingSharedFiles) {
...
async function loadSharedFilesIfAny() {
  ...
  // Si por algún motivo ya estamos en el Paso 1 (lo más probable en un
  // arranque fresco), disparamos directo. Si no, queda en espera hasta que
  // goToStep(1) lo suelte.
  if (state.step === 1) {
